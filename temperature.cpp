...
const std::string CLIENT_ID("temperature-publish");
...
static int body(mqtt::async_client &client, mqtt::connect_options &connOpts) {
	...
	while(1) {
		std::string str;
		std::cout << "Enter the living room temperature in Fahrenheit (" << mqttSettings::MINTEMP << "-"
			<< mqttSettings::MAXTEMP << ") or q to quit: \n";
		std::cin >> str;
		if (str == "q") {
		...
		try {
			...
			while ((temp < mqttSettings::MINTEMP) || (temp > mqttSettings::MAXTEMP)) {
				std::cout << temp << " is an invalid temperature\n";
				std::cout << "Enter the living room temperature in Fahrenheit (" << mqttSettings::MINTEMP << "-"
		                        << mqttSettings::MAXTEMP << ") or q to quit: \n";
				std::cin >> str;
				if (str == "q") {
